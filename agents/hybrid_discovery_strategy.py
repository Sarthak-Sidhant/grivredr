...
class HybridDiscoveryStrategy:
    ...
    def __init__(
        self, config: Optional[DiscoveryConfig] = None, browser_type: str = "firefox"
    ):
        ...
        if needs_browser_use:
            ...
            try:
                browser_use_result = await self._browser_use_discovery(
                    url, municipality, playwright_insights=playwright_result
                )
                ...
                logger.info(
                    f"✅ Hybrid discovery complete (confidence: {results['confidence']:.2f})"
                )
    ...
    def _should_use_browser_use(self, playwright_result: Dict[str, Any]) -> bool:
        ...
        if confidence < 0.7:
            logger.info(
                f"⚠️  Low confidence ({confidence:.2f}) - Browser Use recommended"
            )
            return True
        ...
        if len(cascading_fields) > 2:
            logger.info(
                f"⚠️  Complex cascading ({len(cascading_fields)} levels) - Browser Use recommended"
            )
            return True
        ...
        if complexity > 0.7:
            logger.info(
                f"⚠️  High complexity ({complexity:.2f}) - Browser Use recommended"
            )
            return True
    ...
    async def _playwright_discovery(
        ...
        if not self._playwright_agent:
            self._playwright_agent = FormDiscoveryAgent(
                headless=False, browser_type=self.browser_type
            )
    ...
    async def _browser_use_discovery(
        self, url: str, municipality: str, playwright_insights: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _merge_discoveries(
        self, playwright_result: Dict[str, Any], browser_use_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        ai_observations = browser_use_result.get("form_data", {}).get(
            "observations", []
        )
